...
sex = st.sidebar.selectbox("Sex", options=["Select", "M", "F"])  # Pick your gender.
chest_pain_type = st.sidebar.selectbox("Chest Pain Type", options=["Select", "ATA", "NAP", "ASY", "TA"])  # Pick chest pain type.
resting_bp = st.sidebar.number_input("Resting Blood Pressure (mmHg) | Normal RBP: > 125 mg/dL", 50, 200, 120)  # Enter your blood pressure.
cholesterol = st.sidebar.number_input("Cholesterol (mg/dL) | Normal Level < 200 mg/dL", 50, 600, 200)  # Enter your cholesterol level.
fasting_bs = st.sidebar.selectbox("Fasting? | Normal Fasting Blood Sugar = 70 – 99 mg/dL", options=["Yes", "No"])  # Did you fast? Pick yes or no.
resting_ecg = st.sidebar.selectbox("Resting ECG", options=["Normal", "ST", "LVH"])  # Pick your resting ECG result.
max_hr = st.sidebar.number_input("Max Heart Rate Achieved | Normal Rate: 220 - Your Age" , 60, 220, 140)  # Enter your max heart rate.
exercise_angina = st.sidebar.selectbox("Exercise Induced Angina", options=["Select","Yes", "No"])  # Did you feel chest pain during exercise? Pick yes or no.
...
exercise_angina_mapping = {"Select":0, "Yes":1, "No":2}
resting_ecg_mapping = {"Normal": 0, "ST": 1, "LVH": 2}  # Map ECG results to numbers.
chest_pain_mapping = {"ATA": 0, "NAP": 1, "ASY": 2, "TA": 3, "Select": 4}  # Map chest pain types to numbers.
